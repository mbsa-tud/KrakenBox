...
class Errors:
    def __init__(self, channel, config, run_id):
        ...
        self.config = config
        self.window_size = self.config.window_size  #default 30batches, means default one window equals 30batches
        ...
        smoothing_window = int(self.config.batch_size * self.config.window_size
                               * self.config.smoothing_perc)  #e.g. 70*11*percent
        ...
        self.p_value =  norm(loc = np.mean(self.e), scale=np.std(self.e)).logpdf(self.e)
        self.e_s = self.p_value #new+
        ...
        np.save(os.path.join('data', run_id, 'raw_errors', '{}.npy'
                             .format(channel.id)),
                np.array(self.e))
        
        np.save(os.path.join('data', run_id, 'pdf_errors', '{}.npy'
                             .format(channel.id)),
                np.array(self.e_s))
    ...
    def process_batches(self, channel):
        ...
        logger.info('n_window {},windowsize {}'.format(self.n_windows, self.window_size))
        ...
        for i in range(0, self.n_windows+1):
            ...
            #at last window, the posterior idx equals the length of y_test
            if i == self.n_windows:
                idx = channel.y_test.shape[0]

            window = ErrorWindow(channel, self.config, prior_idx, idx, self, i)
            # logger.info('prior_idx {},idx {}'.format(prior_idx, idx))

            # each window, find one epsilon
            window.find_epsilon()
            window.find_epsilon(inverse=True)
            logger.info('epsilon found, window.epsilon {},window.epsilon_inv {}'.format(window.epsilon, window.epsilon_inv))
            
            window.compare_to_epsilon(self)
            window.compare_to_epsilon(self, inverse=True)
            logger.info('before pruning, i_anom {},i_anom_inv {}'.format(window.i_anom, window.i_anom_inv))
            ...
            # logger.info('after pruning, i_anom {},i_anom_inv {}'.format(window.i_anom, window.i_anom_inv))
...
class ErrorWindow:
    ...
    def prune_anoms(self, inverse=False):
        ...
        logger.info('E_seq_max {},E_seq_max_sorted {}'.format(E_seq_max, E_seq_max_sorted))

        i_to_remove = np.array([])
        for i in range(0, len(E_seq_max_sorted)-1):
            if (E_seq_max_sorted[i] - E_seq_max_sorted[i+1]) \
                    / E_seq_max_sorted[i] < self.config.p or E_seq_max_sorted[i] > 25: #new+
                i_to_remove = np.append(i_to_remove, np.argwhere(
                    E_seq_max == E_seq_max_sorted[i]))
            else:
                i_to_remove = np.array([])
        
        i_to_remove = np.array([0,1,2])
        logger.info('i_to_remove {}'.format(i_to_remove))
        
        i_to_remove[::-1].sort()


        # delete the i_to_remowve in the E_seq
